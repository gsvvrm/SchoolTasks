...
private const val TAG = "TuesdayFragment"
...
class TuesdayFragment : Fragment() {
    ...
    private lateinit var subjTuesdayRecyclerView : RecyclerView
    private var adapter: TuesdayFragment.SubjTuesdayAdapter? = SubjTuesdayAdapter(emptyList())
    ...
    override fun onCreateView(
        ...
        subjTuesdayRecyclerView = view.findViewById(R.id.subj_tuesday_recycler_view) as RecyclerView
        subjTuesdayRecyclerView.layoutManager = LinearLayoutManager(context)
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        jobViewModel.getSubjectsForTuesday().observe(viewLifecycleOwner, Observer {
                subjects -> subjects?.let {

            Log.i(TAG,"Во вторник ${subjects.size} предметов")
            updateUI(subjects)
        }
        })
    ...
    private inner class SubjTuesdayHolder (view: View) : RecyclerView.ViewHolder(view),View.OnClickListener {
        ...
        //val itemShedSubjJob: TextView = itemView.findViewById(R.id.item_shed_subj_job)
        private var adapter: SubjTuesdayHolder.ItemRecyclerAdapter? = ItemRecyclerAdapter(emptyList())
    ...
    private inner class SubjTuesdayAdapter (var subjects: List<Subject>): RecyclerView.Adapter<SubjTuesdayHolder>() {
        override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): SubjTuesdayHolder {
            val view = layoutInflater.inflate(R.layout.list_item_subject_shedule,parent,false)
            return SubjTuesdayHolder(view)
        }

        override fun onBindViewHolder(holder: SubjTuesdayHolder, position: Int) {
    ...
    private fun updateUI(subjects: List<Subject>){
        adapter = SubjTuesdayAdapter(subjects)
        subjTuesdayRecyclerView.adapter = adapter
    }
