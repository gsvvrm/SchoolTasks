...
private const val TAG = "SaturdayFragment"
...
class SaturdayFragment : Fragment() {
    ...
    private lateinit var subjSaturdayRecyclerView : RecyclerView
    private var adapter: SaturdayFragment.SubjSaturdayAdapter? = SubjSaturdayAdapter(emptyList())
    ...
    override fun onCreateView(
        ...
        subjSaturdayRecyclerView = view.findViewById(R.id.subj_saturday_recycler_view) as RecyclerView
        subjSaturdayRecyclerView.layoutManager = LinearLayoutManager(context)
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        jobViewModel.getSubjectsForSaturday().observe(viewLifecycleOwner, Observer {
                subjects -> subjects?.let {

            Log.i(TAG,"В субботу ${subjects.size} предметов")
            updateUI(subjects)
        }
        })
    ...
    private inner class SubjSaturdayHolder (view: View) : RecyclerView.ViewHolder(view),View.OnClickListener {
        ...
        //val itemShedSubjJob: TextView = itemView.findViewById(R.id.item_shed_subj_job)
        private var adapter: SubjSaturdayHolder.ItemRecyclerAdapter? = ItemRecyclerAdapter(emptyList())
    ...
    private inner class SubjSaturdayAdapter (var subjects: List<Subject>): RecyclerView.Adapter<SubjSaturdayHolder>() {
        override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): SubjSaturdayHolder {
            val view = layoutInflater.inflate(R.layout.list_item_subject_shedule,parent,false)
            return SubjSaturdayHolder(view)
        }

        override fun onBindViewHolder(holder: SubjSaturdayHolder, position: Int) {
    ...
    private fun updateUI(subjects: List<Subject>){
        adapter = SubjSaturdayAdapter(subjects)
        subjSaturdayRecyclerView.adapter = adapter
    }
