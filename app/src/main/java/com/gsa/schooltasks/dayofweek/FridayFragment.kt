...
private const val TAG = "FridayFragment"
...
class FridayFragment : Fragment() {
    ...
    private lateinit var subjFridayRecyclerView : RecyclerView
    private var adapter: FridayFragment.SubjFridayAdapter? = SubjFridayAdapter(emptyList())
    ...
    override fun onCreateView(
        ...
        subjFridayRecyclerView = view.findViewById(R.id.subj_friday_recycler_view) as RecyclerView
        subjFridayRecyclerView.layoutManager = LinearLayoutManager(context)
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        jobViewModel.getSubjectsForFriday().observe(viewLifecycleOwner, Observer {
                subjects -> subjects?.let {

            Log.i(TAG,"В пятницу ${subjects.size} предметов")
            updateUI(subjects)
        }
        })
    ...
    private inner class SubjFridayHolder (view: View) : RecyclerView.ViewHolder(view),View.OnClickListener {
        ...
        //val itemShedSubjJob: TextView = itemView.findViewById(R.id.item_shed_subj_job)
        private var adapter: SubjFridayHolder.ItemRecyclerAdapter? = ItemRecyclerAdapter(emptyList())
    ...
    private inner class SubjFridayAdapter (var subjects: List<Subject>): RecyclerView.Adapter<SubjFridayHolder>() {
        override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): SubjFridayHolder {
            val view = layoutInflater.inflate(R.layout.list_item_subject_shedule,parent,false)
            return SubjFridayHolder(view)
        }

        override fun onBindViewHolder(holder: SubjFridayHolder, position: Int) {
    ...
    private fun updateUI(subjects: List<Subject>){
        adapter = SubjFridayAdapter(subjects)
        subjFridayRecyclerView.adapter = adapter
    }
